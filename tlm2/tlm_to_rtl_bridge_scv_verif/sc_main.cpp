...
int sc_main(int argc, char* argv[]) {
  ...
  try {
    ...
    const std::unique_ptr<testbench_module> testbench{new testbench_module{"testbench"}};
    const std::unique_ptr<memmodel_module> memmodel{new memmodel_module{"memmodel", 255}};
    ...
    testbench->testbench_memmodel_socket.bind(memmodel->testbench_memmodel_socket);
    ...
    queue2pins->up_PIPE(testbench->req_PIPE);
    ...
    pins2queue->down_PIPE(testbench->resp_PIPE);

    reset->rstn_port(resetn);
    testbench->stopsim_port(stopsim);
