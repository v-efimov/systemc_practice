...
void spike_module::spike_thread() {
  ...
  while (keep_sim) {
    ...
    do {
        std::cout << "Curr simtime:" <<  sc_time_stamp() << ", provide \"ADDRESS    DATA    OPERATION(0-read/1-write/2-exit)\" separated by spaces: ";
    ...
    switch (inputoperation) {
        ...
        //execute default when operation is 2
        default: {
            //NOTE: Here in the model we indicate that we want to finish simulation via the standard TLM2.0 socket.
            //We don't want to use additional sc_signal here (this signal is defined in the uncore module).
            //This means that the model will communicate with "external world" via single TLM2.0 socket.
            //This practice allow models to be self-contained and be used as "plug-and-play" components.
            keep_sim = false;
            spike_uncore_trans->set_command(tlm::TLM_IGNORE_COMMAND);
        }
    }
